...
export class FirestoreGenericService<T> {
  ...
  getFirestoreDocument(...paths: string[]): AngularFirestoreDocument<Uploaded<T>> {
    if (paths.length % 2 === 1) {
  ...
  protected getFirestoreCollectionQuery(
    ...
    return this.af.collection(paths.join('/'), queryFn as QueryFn | undefined);
  }
  ...
  private prepareDocForUpload<TM>(d: MaybeModeled<TM>): Uploaded<TM> {
    ...
    if (
      !creation || (
        ...
        !(creation instanceof FieldValue)
      )) {
  ...
  private prepareCacheForNewData(): boolean {
    ...
    console.log('DB:CACHE', `reduced cache from ${oldLength} to ${Object.entries(this.cache)}`);
    return true;
}
        ...
        cached = this.cache[cacheKey] = {
            connection$: doc.snapshotChanges().pipe(
                map((snap) => {
                    ...
                    const data: Modeled<T> = snap.payload.data() as Modeled<T>;
                    data._docId = doc.ref.id;
                    console.log('DB:NEW-DATA', 'get', paths.join('/'));
                    return data;
                }),
                finalize(() => console.log('DB:DISCONNECTED', 'get', paths.join('/'))),
...
protected getAllDocuments(queryFn?: QueryFn<Uploaded<T>>, ...paths: string[]): Observable<Modeled<T>[]> {
    /**
    ...
    return this.getFirestoreCollection(queryFn, ...paths).valueChanges({idField: '_docId'})
        .pipe(
            map((s) => s),
            finalize(() => console.log('DB:DISCONNECTED', `query:${index}`, paths.join('/'))),
        );
}
...
private async waitBeforeUpdate(...paths: string[]): Promise<void> {
  ...
  if (nextUpdate - now > 0) {
      console.log('DB', `waiting ${(nextUpdate - now) / 1000} [s] before updating ${path}`);
  }
  await this.wait(nextUpdate - now);
}

protected async updateDocument(data: Partial<T>, ...paths: string[]): Promise<void> {
